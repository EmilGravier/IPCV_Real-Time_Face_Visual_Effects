...
overlayImages = {
    ...
    'Rainbow': {
        'Frame': cv.imread(r'filter_overlays\rainbow.png', cv.IMREAD_UNCHANGED),
        'Position': (0.1, 2.1),  # Relative position to the face centroid in size scale unit
        'Scale': 2.7},  # Width of the overlay image in size scale unit
        
    'Illuminati1': {
        'Frame': cv.imread(r'filter_overlays\illuminati_triangle_eye.png', cv.IMREAD_UNCHANGED),
        'Position': (-0.5, -0.2),  # Relative position to the face centroid in size scale unit
        'Scale': 0.8},  # Width of the overlay image in size scale unit
    'Illuminati2': {
        'Frame': cv.imread(r'filter_overlays\illuminati_triangle_eye.png', cv.IMREAD_UNCHANGED),
        'Position': (0.5, -0.2),  # Relative position to the face centroid in size scale unit
        'Scale': 0.8},  # Width of the overlay image in size scale unit
...
while True:
    ...
    if faceCentroid:  # We can only interact with the filter if the face centroid is computed
        if fingersUp == 0:
            ...
            frame = overlay_image(overlay_dict=overlayImages, overlay_id='Rainbow', frame=frame, face_centroid=faceCentroid, face_scale=sizeScale)
            frame = overlay_image(overlay_dict=overlayImages, overlay_id='Illuminati1', frame=frame, face_centroid=faceCentroid, face_scale=sizeScale)
            frame = overlay_image(overlay_dict=overlayImages, overlay_id='Illuminati2', frame=frame, face_centroid=faceCentroid, face_scale=sizeScale)
        else:
